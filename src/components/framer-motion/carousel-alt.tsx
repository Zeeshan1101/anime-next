          ...
          return (
            <div key={index} className="h-full w-full">
              <CarouselItem
                ...
                episode={image.nextAiringEpisode?.episode as number}
                airingAt={image.nextAiringEpisode?.airingAt as number}
              />
            </div>
          );
        })}
            ...
            return (
              <motion.button
                ...
                style={
                  {
                    "--media-color":
                      image?.coverImage?.color || "hsl(var(--background))",
                  } as CSSProperties
                }
                onClick={() => {
    ...
    <div className="relative z-10 h-full w-full">
      ...
      <div className="absolute bottom-5 z-20 flex w-full flex-col items-center justify-between px-14 text-center sm:flex-row sm:text-justify">
        <div className="text-white">
          <div className="w-full text-xl font-semibold sm:text-2xl ">{alt}</div>
          <div className="opacity-60 ">{episode && episode} Episodes</div>
        </div>
